...
Globals::Globals(int systemState, int numberOfFiles)
    : systemState(systemState), numberOfFiles(numberOfFiles) {}
...
Globals *Globals::getInstance() {
  ...
  if (pGlobals == nullptr) {
    ...
    std::cout << "Globals object exists, returning existing Globals object"
              << std::endl;
...
int main() {
  ...
  // forgot that Globals class exists, now trying to create other 'Globals'
  // class
