...
private:
  std::string name;
  double currentDuration; // the place where the user is currently watching in
                          // [mins]
  double totalDuration;   // length of movie in [mins]
};

// ============= Implementation =============
// interface class helps define all the actions the "Abstract" class should
// define. class should only have a bunch of functional operation's 'blueprints'
class Device {
public:
...
public:
  explicit TVDevice(Movie *movie = nullptr, bool isPlaying = 0,
                    bool isMuted = 1, bool isPowered = 0, int volume = 1,
                    int channel = 1);
...
public:
  explicit RadioDevice(bool isMuted = 1, bool isPowered = 0, int volume = 1,
                       int channel = 1);
...
// ============= Abstraction =============
// class defines the controls and uses the "Implementation" class's variables
// and functions class delegates its work to the "implementation" class
class Remote {
public:
