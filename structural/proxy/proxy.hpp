...
public:
  explicit CreditCardOwnerData(int securityCode = 0, int cardNumber = 0,
...
public:
  explicit CreditCardData(bool isPaymentAuthenticated = false,
                          int validMonth = 0, int validYear = 0,
                          int securityCode = 0, int cardNumber = 0,
                          const std::string &firstName = nullptr,
                          const std::string &lastName = nullptr,
                          const std::string &companyName = nullptr);
  ~CreditCardData();
...
public:
  ...
  virtual double PayAmount(double payment, Time *time,
                           CreditCardOwnerData *creditCardOwnerData) = 0;
};

// ======= Real Subject =======
class Cash : public PaymentType {
public:
  explicit Cash(double paymentBalance = 0.0, double paymentTotal = 0.0);
  ~Cash() override;

  double CheckBalance() override;
  double PayAmount(double payment, Time *time,
                   CreditCardOwnerData *creditCardOwnerData) override;
...
private:
  bool
  CheckPaymentAuthentication(CreditCardData const *creditCardData,
                             Time const *time,
                             CreditCardOwnerData const *creditCardOwnerData);
  bool CheckValidMonth(CreditCardData const *creditCardData,
                       CreditCardOwnerData const *creditCardOwnerData);
  bool CheckValidYear(CreditCardData const *creditCardData, Time const *time,
                      CreditCardOwnerData const *creditCardOwnerData);
  bool CheckSecurityCode(CreditCardData const *creditCardData,
                         CreditCardOwnerData const *creditCardOwnerData);
  bool CheckCardNumber(CreditCardData const *creditCardData,
                       CreditCardOwnerData const *creditCardOwnerData);
  bool CheckFirstName(CreditCardData const *creditCardData,
                      CreditCardOwnerData const *creditCardOwnerData);
  bool CheckLastName(CreditCardData const *creditCardData,
                     CreditCardOwnerData const *creditCardOwnerData);
  bool CheckCompanyName(CreditCardData const *creditCardData,
                        CreditCardOwnerData const *creditCardOwnerData);
};
