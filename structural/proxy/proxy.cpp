...
Time::~Time() { printToConsole("Time destroyed"); }

// ======= CreditCardOwnerData =======
CreditCardOwnerData::CreditCardOwnerData(int securityCode, int cardNumber,
                                         const std::string &firstName,
                                         const std::string &lastName,
                                         const std::string &companyName)
    : securityCode(securityCode), cardNumber(cardNumber), firstName(firstName),
...
// ======= CreditCardData =======
CreditCardData::CreditCardData(bool isPaymentAuthenticated, int validMonth,
                               int validYear, int securityCode, int cardNumber,
                               const std::string &firstName,
                               const std::string &lastName,
                               const std::string &companyName)
    : isPaymentAuthenticated(isPaymentAuthenticated), validMonth(validMonth),
      validYear(validYear), securityCode(securityCode), cardNumber(cardNumber),
...
Cash::Cash(double paymentBalance, double paymentTotal)
    : paymentBalance(paymentBalance), paymentTotal(paymentTotal) {
  printToConsole("Cash created");
}

Cash::~Cash() { printToConsole("Cash destroyed"); }
...
double Cash::PayAmount(double payment, Time *time,
  ...
  // checks for overpayment, returns overplayment else return no overplayment
  // value
  if (newBalance < 0) {
    newBalance = abs(newBalance);
    SetPaymentBalance(0);
    std::cout << "Over payment! Returning $" << std::setprecision(2)
              << newBalance << "!" << std::endl;
  } else {
...
// ======= Credit Card =======
CreditCard::CreditCard(Cash const *cash, CreditCardData *creditCardData,
                       Time *time)
    : cash(new Cash(*cash)), creditCardData(creditCardData), time(time) {
...
CreditCard::CreditCard(const CreditCard &creditCard)
    : cash(creditCard.cash), creditCardData(creditCard.creditCardData),
...
double CreditCard::CheckBalance() { return GetCash()->CheckBalance(); }
...
bool CreditCard::CheckPaymentAuthentication(
    CreditCardData const *creditCardData, Time const *time,
...
bool CreditCard::CheckValidMonth(
...
bool CreditCard::CheckValidYear(
    CreditCardData const *creditCardData, Time const *time,
    CreditCardOwnerData const *creditCardOwnerData) {
...
bool CreditCard::CheckSecurityCode(
    CreditCardData const *creditCardData,
    CreditCardOwnerData const *creditCardOwnerData) {
  if (creditCardData->GetSecurityCode() >= 0 &&
      creditCardData->GetSecurityCode() <= 999 &&
      creditCardData->GetSecurityCode() ==
          creditCardOwnerData->GetSecurityCode()) {
    return true;
  } else
    return false;
}
...
bool CreditCard::CheckFirstName(
    CreditCardData const *creditCardData,
    CreditCardOwnerData const *creditCardOwnerData) {
...
bool CreditCard::CheckCompanyName(
    CreditCardData const *creditCardData,
    CreditCardOwnerData const *creditCardOwnerData) {
...
int main() {
  ...
  CreditCardOwnerData *creditCardOwnerData =
      new CreditCardOwnerData(420, 123456, "John", "Doe", "Capital One");
  CreditCardData *creditCardData = new CreditCardData(
      false, 9, 2026, 420, 123456, "John", "Doe", "Capital One");
  ...
  auto paymentRemainding =
      PayBill(creditCard, 60.00, time, creditCardOwnerData);
