...
Circle::~Circle() { std::cout << "Circle destroyed" << std::endl; }
...
Triangle::~Triangle() { std::cout << "Triangle destroyed" << std::endl; }
...
double Triangle::CalculateHypotenuse() {
  return abs(sqrt(pow(GetSide1(), 2) + pow(GetSide2(), 2) -
                  2 * GetSide1() * GetSide2() * cos(GetAngle())));
}
...
ShapeAdapter::ShapeAdapter(AbstractShapes *abstractShapes)
    : pTriangle(nullptr), pCircle(nullptr) {
...
Triangle *ShapeAdapter::ConvertCircleToTriangle(Circle const *circle) {
  ...
  double triangleSideCalculation =
      sqrt(2 * triangleArea); // Area = a * b / 2 where a = b, because right
                              // triangle legs are the same
...
void ShapeAdapter::SetCircle(Circle *pCircle) { this->pCircle = pCircle; }

void ShapeAdapter::SetTriangle(Triangle *pTriangle) {
  this->pTriangle = pTriangle;
}

Circle *ShapeAdapter::GetCircle() { return this->pCircle; }

Triangle *ShapeAdapter::GetTriangle() { return this->pTriangle; }
