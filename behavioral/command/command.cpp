...
// ============ TV ============
TV::TV(bool isPowered, int channel, int volume)
    : isPowered(isPowered), channel(channel), volume(volume) {}
...
void TV::volumeUp() {
  if (getIsPowered() && getVolume() < MAXVOLUME) {
    ...
    printToConsole("Can change volume -- TV is powered off!");
}
...
void TV::volumeDown() {
  if (getIsPowered() && getVolume() > MINVOLUME) {
    ...
    printToConsole("TV not powered -- can't decrease volume!");
}
...
void TV::channelUp() {
  if (getIsPowered() && getChannel() < MAXCHANNEL) {
    ...
    printToConsole("TV not powered -- can't increase the channel!");
}
...
// ============ PowerOn ============
PowerOn::PowerOn(TV *tv) : tv(tv) {}

void PowerOn::execute() { getTV()->powerOn(); }

// ============ PowerOff ============
PowerOff::PowerOff(TV *tv) : tv(tv) {}

void PowerOff::execute() { getTV()->powerOff(); }

// ============ VolumeUp ============
VolumeUp::VolumeUp(TV *tv) : tv(tv) {}

void VolumeUp::execute() { getTV()->volumeUp(); }

// ============ VolumeDown ============
VolumeDown::VolumeDown(TV *tv) : tv(tv) {}

void VolumeDown::execute() { getTV()->volumeDown(); }

// ============ ChannelUp ============
ChannelUp::ChannelUp(TV *tv) : tv(tv) {}

void ChannelUp::execute() { getTV()->channelUp(); }

// ============ ChannelDown ============
ChannelDown::ChannelDown(TV *tv) : tv(tv) {}

void ChannelDown::execute() { getTV()->channelDown(); }

// ============ Invoker ============
TVRemote::TVRemote(TVCommand *tvCommand) : tvCommand(tvCommand) {}

TVCommand *TVRemote::getTVCommand() const { return this->tvCommand; }

void TVRemote::setCommand(TVCommand *tvCommand) { this->tvCommand = tvCommand; }

void TVRemote::command() const { getTVCommand()->execute(); }
