...
// ======== Memento ========
Memento::Memento(std::string name, std::string specialization,
                 std::string state, int level, int health, int mana,
                 const std::string &date, const std::string &time)
    : name(name), specialization(specialization), state(state), level(level),
      health(health), mana(mana), date(date), time(time) {
  printToConsole("Memento created");
}

Memento::~Memento() { printToConsole("Memento deleted"); }

// ======== Originator ========
Originator::Originator(std::string name, std::string specialization,
                       std::string state, int level, int health, int mana)
    : name(name), specialization(specialization), state(state), level(level),
      health(health), mana(mana) {
  // checks value ranges for max/min
  if (getLevel() > MAXIMUM_LEVEL)
    setLevel(MAXIMUM_LEVEL);
  else if (getLevel() < MINIMUM_LEVEL)
    setLevel(MINIMUM_LEVEL);

  if (getHealth() > MAXIMUM_VALUE)
    setHealth(MAXIMUM_VALUE);
  else if (getHealth() < MINIMUM_VALUE)
    setHealth(MINIMUM_LEVEL);

  if (getMana() > MAXIMUM_VALUE)
    setMana(MAXIMUM_VALUE);
  else if (getHealth() < MINIMUM_VALUE)
    setMana(MINIMUM_VALUE);

  printToConsole("Originator created");
}

Originator::~Originator() { printToConsole("Originator deleted"); }

void Originator::changeName(const std::string &newName) { setName(newName); }
...
Memento *Originator::save() {
  return new Memento(this->getName(), this->getSpecialization(),
                     this->getState(), this->getLevel(), this->getHealth(),
                     this->getMana());
}
...
Caretaker::~Caretaker() { printToConsole("Originator deleted"); }

// saves a snapshot of the current state of the originator
void Caretaker::backup() { getMementos().push_back(getOriginator()->save()); }
...
void Caretaker::undo() {
  ...
  try {
    getOriginator()->restore(
        memento); // places last save data into current data
  } catch (...) {
    undo();
  }
}
...
int main() {
  Originator *originator =
      new Originator("George", "Monk", "Happy", 1, 100, 100);
